...
class HomeRemoteDataSource implements IHomeRemoteDataSource {
  ...
  Stream<List<OrderModel>> getUpcomingOrders() {
    ...
    // var list=snapshots
    //     .map((snapshot){
    //       print('saboor :: ${snapshot.docs}');
    //       print('saboor :: ${snapshot.size}');
    // });

        return snapshots
        .map((snapshot){
          print('saboor :docs: ${snapshot.docs}');
          print('saboor :size: ${snapshot.size}');
          return OrderModel.parseListOfDocument(snapshot.docs);
        });
  }
